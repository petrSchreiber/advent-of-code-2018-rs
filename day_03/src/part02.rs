...
pub fn get_nonoverlapping_claim(passed_claims: &std::vec::Vec<claims::Claim>) -> usize {
    ...
    // Detect non-overlapping claim
    'outer: for claim in passed_claims {
        for x in claim.offset_x..claim.offset_x+claim.width {
            for y in claim.offset_y..claim.offset_y+claim.height {
                if grid_raw[x + y*width] > 1 {
                    continue 'outer;
                }
            }
        }
